...
# # Path to data files
# path = r"C:\Users\NITHIN P\OneDrive\Desktop\Chipathon\gmid_GF180MCUD\PMOS_06v0_SVT\PMOS_06v0_SVT\\"

# # Constants and initialization
# W = 3e-6  # Width in meters
# num_files = 18
...
#     filename = path + f"gmid_pmos_{i + 1}_pfet_06v0_svt_tb.txt"
#     try:
...
#                         # Filter bad data
#                         if id_val < 1e-9 or gds == 0 or Cgg == 0:
#                             continue

#                         vov_value = Vgs_val - Vth

#                         # Vov condition
#                         if -0.5 < vov_value < 6.0:
#                             gm_id_val = gm / id_val

#                             # gm/id spike protection
#                             if abs(gm_id_val) > 100:
#                                 continue

#                             vgs[i].append(Vgs_val)
#                             Vov[i].append(vov_value)
#                             gm_id[i].append(gm_id_val)
...
# # Labels
# labels = ['0.75u', '1u', '1.25u', '1.5u', '1.75u', '2u', '2.25u', '2.5u', '2.75u',
#           '3u', '3.25u', '3.5u', '3.75u', '4u', '4.25u', '4.5u', '4.75u', '5u']
# min_length = min(len(vgs), len(gm_id), len(labels))

# # Check lengths
# for idx in range(min_length):
#     print(f"Index {idx}: vgs={len(vgs[idx])}, gm_id={len(gm_id[idx])}")
...
#         line, = ax.plot(x_data[i], y_data[i], linestyle='-', color=colormap(i % 20), linewidth=2, label=f"L={labels[i]}")
#         lines.append(line)
...
# # Plotting
# plot_with_checkboxes(vgs, gm_id, 'Vgs (V)', 'gm/id', 'PMOS 6V SVT gm/id vs Vgs')
# plot_with_checkboxes(Vov, gm_id, 'Vov (V)', 'gm/id', 'PMOS 6V SVT gm/id vs Vov')
# plot_with_checkboxes(gm_id, gm_gds, 'gm/id', 'gm/gds', 'PMOS 6V SVT gm/gds vs gm/id')
# plot_with_checkboxes(gm_id, id_W, 'gm/id', 'id/W (A/m)', 'PMOS 6V SVT id/W vs gm/id')
# plot_with_checkboxes(gm_id, ft, 'gm/id', 'ft (Hz)', 'PMOS 6V SVT ft vs gm/id')
# plot_with_checkboxes(gm_id, cgd_cgg, 'gm/id', 'Cgd/Cgg', 'PMOS 6V SVT Cgd/Cgg vs gm/id')
# plot_with_checkboxes(gm_id, cgs_cgg, 'gm/id', 'Cgs/Cgg', 'PMOS 6V SVT Cgs/Cgg vs gm/id')

# # Optional: Combined static plots
# def plot_all_together():
#     fig, axs = plt.subplots(3, 3, figsize=(15, 10))
#     fig.suptitle("PMOS 6V SVT gm/id Plots", fontsize=16, fontweight='bold')
#     plot_data = [
#         (vgs, gm_id, 'Vgs', 'gm/id'),
#         (Vov, gm_id, 'Vov', 'gm/id'),
...
plot_with_checkboxes(vgs, gm_id, 'Vgs', 'gm/id', 'PMOS 6V SVT  gm/id vs Vgs')

# Plot gm/id versus Vov
plot_with_checkboxes(Vov, gm_id, 'Vov', 'gm/id', 'PMOS 6V SVT  gm/id vs Vov')

# Plot gm/gds versus gm/id
plot_with_checkboxes(gm_id, gm_gds, 'gm/id', 'gm/gds', 'PMOS 6V SVT  gm/gds vs gm/id')

# Plot id/W versus gm/id
plot_with_checkboxes(gm_id, id_W, 'gm/id', 'id/W', 'PMOS 6V SVT  id/W vs gm/id')
...
def plot_all_together():
    fig, axs = plt.subplots(3, 3, figsize=(15, 10))
    fig.suptitle("PMOS 6V SVT gm/id plots", fontsize=16, fontweight='bold')  # Title added
    plot_data = [
        (vgs, gm_id, 'Vgs', 'gm/id'),
        (Vov, gm_id, 'Vov', 'gm/id'),
        (gm_id, gm_gds, 'gm/id', 'gm/gds'),
